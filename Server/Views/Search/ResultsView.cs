...
internal class ResultsView : AbstractView
{
    ...
    private ISearchService SearchEngine => new SqliteSearchService(Settings.Global.SearchDbFile);

    ArticleSummary? TopGemipediaHit;
    IReadOnlyList<TextSearchResult>? SearchResults;
    int ImageHits = 0;
    ...
    private void WriteResultEntry(TextSearchResult result, int resultNumber)
    {
    ...
    {
        int baseCounter = Options.SearchPage - 1;
        SearchResults = SearchEngine.SearchText(query, baseCounter * resultsInPage, resultsInPage);
    }
